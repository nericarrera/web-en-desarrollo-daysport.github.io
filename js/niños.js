...
document.addEventListener('DOMContentLoaded', () => {
    ...
    niñosProductsGrid.insertAdjacentElement('afterend', loadMoreBtn);
    ...
    function mostrarProductos(categoria = "all", coloresSeleccionados = [], tallesSeleccionados = [], orden = "", resetPagination = true) {
        if (resetPagination) {
            currentPage = 1;
            niñosProductsGrid.innerHTML = ""; // Solo limpiamos si es un nuevo filtro
        }
    ...
    loadMoreBtn.addEventListener('click', () => {
        currentPage++;
        mostrarProductos(
            document.querySelector('.niños-filter-button.active')?.getAttribute('data-filter') || "all",
    ...
    if (applyFiltersButton) {
        applyFiltersButton.addEventListener('click', () => {
            const selectedCategory = document.querySelector('.niños-filter-button.active')?.getAttribute('data-filter') || "all";
    ...
    clearFiltersButton.addEventListener('click', () => {
        ...
        filterButtons.forEach(btn => btn.classList.remove('active'));
        document.querySelector('.niños-filter-button[data-filter="all"]').classList.add('active');
